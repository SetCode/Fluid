...
namespace Anda.Fluid.Drive.SetupCard
{
    ...
    /// 4轴Gts卡，R轴：针嘴旋转轴
    /// </summary>
    internal class CardSetupGts4_XYZR : ICardSetupable
    {
        public RobotXYZ Setup()
        {
            ...
            Axis axisR = new Axis((int)AxisType.R轴, card, 4, AxisType.R轴.ToString(), AxisPrmMgr.Instance.FindBy(4));
            AxisMgr.Instance.Add(axisX);
            AxisMgr.Instance.Add(axisY);
            AxisMgr.Instance.Add(axisZ);
            AxisMgr.Instance.Add(axisR);
            return new RobotXYZ(axisX, axisY, axisZ, axisR, RobotAxesStyle.XYZR);
        }
