...
namespace Anda.Fluid.Domain.FluProgram.Executant.Fluider.ValveTracker.JtValveTracker
{
     ...
     */
    public class JtValveArc : ArcTrackBase
    {
        ...
        private Result ExecuteLogic(Arc arc)
        {
            ...
            // 以相机为中心的坐标转换成以喷嘴为中心
            //PointD realStart = Converter.CameraToNeedle1(start);
            //PointD realMiddle = Converter.CameraToNeedle1(middle);
            //PointD realEnd = Converter.CameraToNeedle1(end);
            //PointD realCenter = Converter.CameraToNeedle1(center);
        ...
        private Result executeArc(Arc arc,PointD accStart, PointD arcStart, PointD arcEnd, PointD decEnd, PointD center, short clockwise, double vel, PointD[] points, double intervalSec)
        {
            ...
            //{
            //    newStart = Converter.Needle1ToCamera(arcStart);
            //    newEnd = Converter.Needle1ToCamera(arcEnd);
            //    newAccStart = Converter.Needle1ToCamera(accStart);
            //    newDecEnd = Converter.Needle1ToCamera(decEnd);
            //    newCenter = Converter.Needle1ToCamera(center);
            //    for (int i = 0; i < points.Length; i++)
            //    {
            ...
            else
            {
                if (currZ > targZ)
                {
                    ...
                    ret = Machine.Instance.Robot.MovePosZAndReply(z, arc.Param.DownSpeed, arc.Param.DownAccel);
                    if (!ret.IsOk)
                    {
                        return ret;
                    }
            ...
            {
                ret = Machine.Instance.Valve1.FluidArc(cameraAccStart, cameraStart, cameraEnd, cameraDecEnd, cameraCenter, clockwise, vel, cameraPoints, intervalSec, FluidProgram.Current.MotionSettings.WeightAcc);
                GlueManagerMgr.Instance.UpdateGlueRemainWeight(0, points.Length * arc.Program.RuntimeSettings.SingleDropWeight);
            }
            if (!ret.IsOk)
            {
