...
{
    public class AxisYCPK:CpkBase
    {
        public AxisYCPK(Sheet sheet, CpkPrm prm) : base(sheet, prm.AxisYSpec)
        {
            this.CpkPrm = prm;
        ...
        public PointD calPoint()
        {
            ...
            double distanceY = yl / 10;
            pointTrans.Y = this.End.Y - distanceY*3;
            pointTrans.X = this.End.X;
        ...
        private bool CaptruePos(out double pos)
        {
            ...
            //像素坐标转机械坐标
            pos = Machine.Instance.Camera.ToMachine(outX, outY).Y;
            return result;
        }
        ...
        {
            //CPK测试报告(Y轴定位精度)
            sheet.SetOneCellValue(1, "A", "CPK测试报告(Y轴定位精度)");
