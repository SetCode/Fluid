...
{
    public partial class EditPatternsArrayMetro : MetroSetUserControl, IMsgSender, ICanSelectButton
    {
        enum RepeatMode
        {
            X向蛇行,
        ...
        /// 仅用于生成语言文本
        /// </summary>
        private EditPatternsArrayMetro()
        {
            InitializeComponent();
        }

        public EditPatternsArrayMetro(Pattern pattern)
        {
            InitializeComponent();
        ...
        private void btnOk_Click(object sender, System.EventArgs e)
        {
            ...
            if (tbHNums.Value < 1)
            {
                ...
                MetroSetMessageBox.Show(this, "横向拼版数不可以小于 1.");
                return;
            }
            if (tbOriginX.Value == tbHEndX.Value && tbOriginY.Value == tbHEndY.Value)
            {
            ...
            if (tbHEndX.Value == tbVEndX.Value && tbHEndY.Value == tbVEndY.Value)
            {
                ...
                MetroSetMessageBox.Show(this, "横向的终点坐标和纵向的终点坐标不可以相同.");
                return;
            }
            //机械坐标->系统坐标
            PointD pO = this.pattern.MachineRel(tbOriginX.Value, tbOriginY.Value);
