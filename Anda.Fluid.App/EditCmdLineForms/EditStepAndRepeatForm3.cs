...
namespace Anda.Fluid.App.EditCmdLineForms
{
    ...
    /// 单拼版阵列指令窗口
    /// </summary>
    public partial class EditStepAndRepeatForm3 : EditFormBase, IMsgSender
    {
        ...
        private bool noRecursion = false;

        public EditStepAndRepeatForm3(Pattern pattern) : this(pattern, null)
        {
        }

        public EditStepAndRepeatForm3(Pattern pattern, StepAndRepeatCmdLine stepAndRepeatCmdLine) : base(pattern.GetOriginPos())
        {
            InitializeComponent();
            this.loadFirst = true;
            ...
            if (!this.isCreating)
            {
                for (int i = 0; i < this.stepAndRepeatCmdLine.DoCmdLineList.Count; i++)
                {
                    ...
                    patternItems[i].BoardNo = stepAndRepeatCmdLine.DoCmdLineList[i].BoardNo;
                }
            }
            this.lstDoPatterns.MouseDown += LstDoPatterns_MouseDown;
            this.txtBoardNo.TextChanged += TxtBoardNo_TextChanged;
        ...
        private void TxtBoardNo_TextChanged(object sender, EventArgs e)
        {
            if (this.noRecursion)
            {
                return;
            }
            this.patternItems[lstDoPatterns.SelectedIndex].BoardNo = this.txtBoardNo.Value;
        }
        ...
        private void LstDoPatterns_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                this.cmsValveType.Show(this.gbx1.PointToScreen(new Point(this.lstDoPatterns.Location.X + e.Location.X,
        ...
        /// 仅用于生成语言文本
        /// </summary>
        private EditStepAndRepeatForm3()
        {
            InitializeComponent();
        }
        ...
        private void lstDoPatterns_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            this.txtBoardNo.Text = patternItems[this.selectedIndex].BoardNo.ToString();
        ...
        private void btnOk_Click(object sender, System.EventArgs e)
        {
            ...
            if (tbHNums.Value < 1)
            {
                ...
                MessageBox.Show("横向拼版个数不可以小于1.");
                return;
            }
            if (tbHNums.Value * tbVNums.Value == 1)
            {
                MessageBox.Show("拼版数量不可以等于1.");
                return;
            }
            if (tbOriginX.Value == tbHEndX.Value && tbOriginY.Value == tbHEndY.Value)
            {
            ...
            {
                DoCmdLine doCmdLine = new DoCmdLine(stepAndRepeatCmdLine.PatternName, item.Point.X, item.Point.Y) { Enabled = item.Enabled, Reverse = item.Reverse,BoardNo = item.BoardNo };
        ...
        class PatternItem
        {
            ...
            public override string ToString()
            {
                ...
                builder.Append(string.Format("[{0},{1}] {2} {3} Board {4} {5}", I, J, Point, valveDisplay,BoardNo, Reverse == true ? "Rev" : ""));
            ...
            if (this.patternItems != null && this.loadFirst)
            {
                ...
                if (this.stepAndRepeatCmdLine.HorizontalNums * this.stepAndRepeatCmdLine.VerticalNums == this.lstDoPatterns.Items.Count)
                {
                    ...
                    for (int i = 0; i < this.lstDoPatterns.Items.Count; i++)
                    {
                        ...
                        patternItems[i].BoardNo = this.stepAndRepeatCmdLine.DoCmdLineList[i].BoardNo;
                    }
                    this.showCklistBox();
                }
            }
